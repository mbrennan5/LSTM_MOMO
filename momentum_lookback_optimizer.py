...
def run_judicial_audit(brain_name, master_df, model_type='GRU',
    ...
    if USE_PERMUTATION_SCORING:
        if USE_FAMILY_GROUPED_PERM:
            groups = _group_features_by_family(feat_cols)
            for family, indices in tqdm(groups.items(), desc="Permutation scoring (family)"):
                try:
                    ...
                    drop = max(0.0, baseline_acc - pa)
                    for fi in indices:
                        rows.append({'Feature': feat_cols[fi], 'I_raw': drop})
                except Exception:
                    for fi in indices:
                        rows.append({'Feature': feat_cols[fi], 'I_raw': 0.0})
        else:
